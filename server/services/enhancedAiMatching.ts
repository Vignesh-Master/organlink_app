...
export class EnhancedAIMatchingService {
  ...
  public async matchPatientToDonors(
    ...
    if (!policy || !policy.is_active) {
      throw new Error(
        `No active policy found for organ type: ${patient.organ_type}`,
      );
    }
    ...
    for (const donor of donors) {
      ...
      // ABO compatibility
      if (
        policy.constraints.require_abo_compat &&
        !this.isABOCompatible(
          patient.blood_group,
          donor.blood_group,
          policy.constraints.use_rh_factor,
        )
      ) {
        filteredCount++;
        continue;
      ...
      if (
        patient.age &&
        donor.age &&
        Math.abs(patient.age - donor.age) >
          policy.constraints.max_age_difference
      ) {
        filteredCount++;
        continue;
      }
      ...
      if (patient.weight && donor.weight) {
        const weightDiff =
          Math.abs(patient.weight - donor.weight) /
          Math.max(patient.weight, donor.weight);
        if (
          weightDiff >
          policy.constraints.max_weight_difference_percent / 100
        ) {
          filteredCount++;
          continue;
        }
      ...
      const bloodScore = this.isABOCompatible(
        patient.blood_group,
        donor.blood_group,
        policy.constraints.use_rh_factor,
      )
      ...
      const distanceScore = Math.max(
        0,
        1 - distance / policy.constraints.max_distance_km,
      );
      const ageScore =
        patient.age && donor.age
          ? Math.max(0, 1 - Math.abs(patient.age - donor.age) / 50)
          : 0.5;
      const weightScore =
        patient.weight && donor.weight
          ? Math.max(
              0,
              1 -
                Math.abs(patient.weight - donor.weight) /
      ...
      // Verification score
      const patientVerificationScore = patient.ocr_verified
        ? (patient.ocr_score_bps || 8000) / 10000
        : 0.5;
      const donorVerificationScore = donor.ocr_verified
        ? (donor.ocr_score_bps || 8000) / 10000
        : 0.5;
      const verificationScore = Math.min(
        patientVerificationScore,
      ...
      if (options.includeMLPrediction && this.trainingData.length > 0) {
        const mlFeatures = {
          ...
          age_diff:
            patient.age && donor.age
              ? Math.abs(patient.age - donor.age) / 50
              : 0.5,
      ...
      const policyCompliance = {
        pediatric_priority:
          !policy.constraints.pediatric_priority ||
          !patient.age ||
          patient.age >= 18 ||
          urgencyScore > 0.8,
        emergency_case:
          !patient.urgency ||
          patient.urgency < policy.thresholds.critical_urgency_threshold,
        geographic_preference:
          distance <= policy.constraints.max_distance_km * 0.5,
        verification_required:
          !policy.constraints.require_blockchain_verification ||
      ...
      if (distance > policy.constraints.max_distance_km * 0.8)
        warnings.push("Long distance transport required");
      if (patient.age && patient.age < 18 && urgencyScore < 0.8)
        warnings.push("Pediatric case with non-critical urgency");
      if (verificationScore < 0.8)
        warnings.push("Document verification below recommended threshold");

      // Apply thresholds
      if (
        finalScore < policy.thresholds.min_match_score ||
