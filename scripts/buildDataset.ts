    ...
    out.push({
      ...
      outcome:
        String(r["Match_Status"] || "")
          ...
          ? 1
          : 0,
      height: r["Donor_Height"],
      weight: r["Donor_Weight"],
      city: r["Donor_Location"],
      state: undefined,
      country: undefined,
...
function buildBrazil(rows: Row[]): Row[] {
  return rows.map((r) => ({
    ...
    outcome: (() => {
      const v = String(r["Transplant_Y_N"] ?? "")
        .toLowerCase()
        .trim();
...
function stringifyCsv(rows: Row[]): string {
  ...
  const normalized = rows.map((r) => {
    ...
    if (allMissing) {
      ...
      r["HLA_missing"] = hla.every(
        (x) => String(x || "").toUpperCase() === "UNKNOWN",
      )
        ? 1
        : 0;
    }
...
function enhanceDataset(rows: Row[]): Row[] {
  return rows.map((r, index) => {
    ...
    r.blockchain_timestamp =
      Math.floor(Date.now() / 1000) - Math.floor(Math.random() * 86400 * 30); // Last 30 days
    ...
    r.location_tier = r.city
      ? ["Mumbai", "Delhi", "Chennai", "Bangalore"].includes(r.city)
        ? "tier1"
        : Math.random() > 0.5
          ? "tier2"
          : "tier3"
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  const out =
    (args["out"] as string) || "data/clean/organlink_training_data.csv";
  const enhance = args["enhance"];

  if (!brazil && !supply) {
    console.error("Usage:");
    console.error(
      "  npx tsx scripts/buildDataset.ts --brazil <path> --supply <path> [--org <path>] --out <path> [--enhance]",
    );
    console.error("");
  ...
  console.log(
    `🔧 Normalizing and deduplicating ${merged.length} total records...`,
  );
